 ...
 */
function proxyRequest(req, res, proxy: HttpProxy) {
...
interface HandlerOptions {
  basePath?: string;
  getProxyForUrl?: typeof getProxyForUrl; // Function that specifies the proxy to use
  maxRedirects?: number; // Maximum number of redirects to be followed.
  ...
  checkRateLimit?: (origin: string) => undefined | string; // Function that may enforce a rate-limit by returning a non-empty string.
  redirectSameOrigin?: false; // Redirect the client to the requested URL for same-origin requests.
  requireHeader?: string[]; // Require a header to be set?
  removeHeaders?: string[]; // Strip these request headers.
  setHeaders?: { [name: string]: string }; // Set these request headers.
...
function getHandler(options: HandlerOptions, proxy: HttpProxy) {
  const corsAnywhere = {
  ...
  const hasRequiredHeaders = (headers) => {
    return (
      ...
      corsAnywhere.requireHeader.some((headerName) => {
  ...
  return (req, res) => {
    ...
    const location = parseURL(req.url.replace(options.basePath, "").slice(1));
    if (!location) {
...
export default function proxyMiddleware(options: HandlerOptions = {}) {
  // Default options:
  ...
  proxy.on("proxyRes", (proxyRes, req, res) => {
    modifyResponse(res, proxyRes, options.removeHeaders, (body) => {
      if (body) {
        body = rewriteLink(
          body,
          new URL("http://localhost:3000/plugins/iframe/")
        );
      }
      return body;
    });
